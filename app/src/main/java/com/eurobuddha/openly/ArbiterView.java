...
/**
 * Arbiter tab — placeholder shell (Phase 1). Real content arrives in Phase 7.
...
public class ArbiterView extends BaseView {
    ...
    private static int makeRoot(MainActivity a) {
        return android.R.layout.simple_list_item_1; // temporary host; replaced in Phase 7
    }

    @Override public void refresh() {
        if (body == null) return;
        body.setGravity(Gravity.CENTER);
        body.setText("Arbiter\n\nOpenly ready\nblock #" + act.block()
                + "\nbalance " + act.balance
